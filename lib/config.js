...
const COOLDOWN_SECONDS = parseInt(process.env.COOLDOWN_SECONDS || '30', 10); // anti-spam
...
// ---- Objets d'inventaire spéciaux ----
// Banane Triple : lance sur 2-3 cibles en un coup (mêmes probas/points que le tir classique, pas de stat dédiée).
// Banane Stop : bloque une cible pendant STUN_DURATION_SECONDS (elle ne peut plus lancer).
const ITEM_ID_TRIPLE = process.env.ITEM_ID_TRIPLE || 'ban_tripple'; // ID de l'objet dans le Google Sheet
const ITEM_ID_STOP = process.env.ITEM_ID_STOP || 'ban_stop';       // ID de l'objet dans le Google Sheet
const STUN_DURATION_SECONDS = parseInt(process.env.STUN_DURATION_SECONDS || '120', 10); // durée du blocage Banane Stop

// ---- Badges spéciaux (uniques à une personne précise, visuel seulement) ----
// La personne garde toutes les permissions de son rôle réel (mod, ici) — c'est
// juste l'icône/le texte affichés sur son profil qui changent. Pour en ajouter
// un autre, il suffit de rajouter une ligne ici (clé = pseudo en minuscules).
const SPECIAL_BADGES = {
  raven_spectral: 'supermod', // "Modaque" 🐒🛡️ — badge unique, remplace l'affichage du badge Modérateur
};
